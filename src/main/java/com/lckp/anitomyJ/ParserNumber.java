...
import static com.lckp.anitomyJ.Element.ElementCategory.kElementAnimeSeason;
...
import static com.lckp.anitomyJ.Element.ElementCategory.kElementEpisodeNumberAlt;
...
public class ParserNumber {
    ...
    public boolean matchMultiEpisodePattern(String word, Token token) {
        String regexPattern = "(\\d{1,4})(?:[vV](\\d))?[-~&+](\\d{1,4})(?:[vV](\\d))?$";
        Pattern pattern = Pattern.compile(regexPattern);
        Matcher matcher = pattern.matcher(word);
        if (matcher.matches()) {
